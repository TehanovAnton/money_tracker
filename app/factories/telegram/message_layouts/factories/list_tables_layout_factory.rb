...
module Telegram
  module MessageLayouts
        module Factories
          class ListTablesLayoutFactory < ::Abstract::AbstractFactory
            def execute; end

            def input_parser_factory(factory_name)
              Telegram::MessageLayouts::Factories::ListTablesLayoutInputParserFactory.run!(
                factory_name: factory_name, style: :initializer
              )
            end

            def text_preparation_factory(factory_name, text)
              Telegram::MessageLayouts::Factories::ListTablesLayoutTextPreparationFactory.run!(
            ...
            def layout_params_factory(factory_name, parsed_input)
              Telegram::MessageLayouts::Factories::ListTablesLayoutParamsFactory.run!(
                factory_name: factory_name,
